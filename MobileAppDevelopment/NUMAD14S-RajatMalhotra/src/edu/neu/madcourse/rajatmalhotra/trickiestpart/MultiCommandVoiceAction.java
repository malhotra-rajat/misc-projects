	...
	public boolean interpret(List<String> heard, float[] confidenceScores) {
		...
		for (int i = 0; i < heard.size() && !understood; i++) {
			...
			if (hasConfidenceScores) {
				exceedsMinConfidence = (confidenceScores[i] > getMinConfidenceRequired());
			}
			if (exceedsMinConfidence) {
				WordList saidWords = new WordList(said);
				for (VoiceActionCommand command : commands) {
					understood = command.interpret(saidWords, confidenceScores);
					if (understood) {
		...
		if (!understood) {
			if (hasConfidenceScores) {
				...
				// interpret confidence so as to provide a reason to
				// notUnderstood
				...
				if (highestConfidenceScore < 0.0) {
					...
					if (highestConfidenceScore < getInaccurateConfidenceThreshold()) {
						getNotUnderstood()
								.notUnderstood(
										heard,
										OnNotUnderstoodListener.REASON_INACCURATE_RECOGNITION);
					} else if (highestConfidenceScore >= getNotACommandConfidenceThreshold()) {
						getNotUnderstood().notUnderstood(heard,
